...
def predict_note_authentication(age, bmi, fpg, ffpg, sbp,dbp,family_history, smoking, drinking, alt):
...
with col1:
    ...
    bmi = st.number_input("bmi", min_value=0, max_value=30, key="bmi", placeholder="Type Here")
    fpg = st.number_input("fpg", min_value=0, max_value=100, key="fpg", placeholder="Type Here")
    ffpg = st.number_input("ffpg", min_value=0.0, max_value=40.0, key="ffpg", placeholder="Type Here")
    sbp = st.number_input("sbp", min_value=20, max_value=150, key="sbp", placeholder="Type Here")

# col2
with col2:    
    dbp = st.number_input("dbp", min_value=1, max_value=100, key="dbp", placeholder="Type Here")
    family_history = st.number_input("family_history", min_value=0, max_value=10, key="family_history", placeholder="Type Here")
    smoking = st.number_input("smoking", min_value=0, max_value=4, key="smoking", placeholder="Type Here")
    drinking = st.number_input("drinking", min_value=0, max_value=4, key="drinking", placeholder="Type Here")
    alt = st.number_input("alt", min_value=1, max_value=180, key="alt", placeholder="Type Here")
